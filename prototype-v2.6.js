...
var curPage='home',openGroup='workspace',curTab='全部',curSubTab='按日统计',toastTimer,currentRole='admin',grabCounter=0,assetDateFrom='',assetDateTo='';
...
function showOrderDetail(oid){
  ...
  var timelineHTML='<div style="border-top:1px solid var(--border-light);padding-top:16px;margin-top:8px;">'+
    ...
    '<div style="position:relative;margin-bottom:16px;"><div style="position:absolute;left:-20px;top:4px;width:10px;height:10px;border-radius:50%;background:var(--primary);border:2px solid #fff;box-shadow:0 0 0 2px var(--primary);"></div><div style="font-size:13px;color:var(--text-secondary);">2026-05-26 14:30</div><div style="font-weight:500;">录入订单</div><div style="font-size:13px;color:var(--text-secondary);">操作人：张运营</div><div style="margin-top:4px;"><a class="link" onclick="toast(\'已查看截图\')" style="font-size:13px;">📷 查看截图 (2张)</a></div></div>'+
    ...
    '<div style="position:relative;margin-bottom:4px;"><div style="position:absolute;left:-20px;top:4px;width:10px;height:10px;border-radius:50%;background:var(--primary);border:2px solid #fff;box-shadow:0 0 0 2px var(--primary);"></div><div style="font-size:13px;color:var(--text-secondary);">2026-05-29 10:00</div><div style="font-weight:500;">结算</div><div style="font-size:13px;color:var(--text-secondary);">操作人：张运营</div><div style="font-size:13px;">结算金额：¥298.00（代练分成 ¥208.00）</div></div>'+
    '</div></div>';
...
function rMyAssets(){
  ...
  h+='<div class="ant-card"><div class="ant-card-head">💳 资产明细流水</div><div class="ant-card-body np"><div class="ant-table-wrap"><table class="ant-table"><thead><tr><th>时间</th><th>类型</th><th>关联订单</th><th>金额变动</th><th>余额</th>'+(isGuild?'<th>代练</th>':'')+'<th>备注</th></tr></thead><tbody>';
  var flows;
  if(isGuild){
    flows=[{time:'05-26 14:30',type:'订单收入',tag:'ant-tag-primary',order:'DD202605260015',chg:'+¥298',bal:'¥24,880.00',booster:'王代练',note:'跑刀 300万哈夫币'},{time:'05-26 11:15',type:'订单收入',tag:'ant-tag-primary',order:'DD202605260013',chg:'+¥520',bal:'¥24,582.00',booster:'刘代练',note:'装备刷取x3'},{time:'05-25 18:30',type:'订单收入',tag:'ant-tag-primary',order:'DD202605250010',chg:'+¥650',bal:'¥24,062.00',booster:'陈代练',note:'账号练级1→30'},{time:'05-25 10:00',type:'结算打款',tag:'ant-tag-warning',order:'-',chg:'-¥12,500',bal:'¥23,412.00',booster:'-',note:'机构结算打款 05月批次'},{time:'05-24 15:20',type:'订单收入',tag:'ant-tag-primary',order:'DD202605240005',chg:'+¥350',bal:'¥35,912.00',booster:'王代练',note:'排位黄金→铂金'},{time:'05-24 09:00',type:'订单收入',tag:'ant-tag-primary',order:'DD202605240003',chg:'+¥800',bal:'¥35,562.00',booster:'赵代练',note:'排位钻石→黑鹰'},{time:'05-23 16:00',type:'订单收入',tag:'ant-tag-primary',order:'DD202605230008',chg:'+¥300',bal:'¥34,762.00',booster:'王代练',note:'排位青铜→钻石'},{time:'05-22 12:00',type:'服务费支出',tag:'ant-tag-default',order:'-',chg:'-¥500',bal:'¥34,462.00',booster:'-',note:'平台服务费月结'}];
  }else{
  ...
  var displayFlows=flows;
  if(assetDateFrom||assetDateTo){
    displayFlows=flows.filter(function(f){var d=f.time.split(' ')[0];if(assetDateFrom&&d<assetDateFrom)return false;if(assetDateTo&&d>assetDateTo)return false;return true;});
  }
  displayFlows.forEach(function(f){
    var cls=f.chg.indexOf('+')===0?' style="color:var(--success);"':' style="color:var(--danger);"';
    h+='<tr><td>'+f.time+'</td><td><span class="ant-tag '+f.tag+'">'+f.type+'</span></td><td class="link">'+f.order+'</td><td'+cls+'>'+f.chg+'</td><td>'+f.bal+'</td>'+(isGuild?'<td>'+(f.booster||'-')+'</td>':'')+'<td>'+f.note+'</td></tr>';
  });
...
function exportMyAssetFlow(){
  ...
  var headers=['时间','类型','关联订单','金额变动','余额','备注'];
  if(document.querySelector('#content-area .ant-table th:nth-child(6)')&&document.querySelector('#content-area .ant-table th:nth-child(6)').textContent==='代练'){headers.splice(5,0,'代练');}
  exportCSV(headers,rows,'资产明细_'+new Date().toISOString().slice(0,10)+'.csv');
}
// ── 平台资产（平台管理员） ──
...
function openGuildDetail(idx){
  ...
  // Order table
  h+='<div style="margin-bottom:20px;"><div style="font-weight:600;margin-bottom:8px;">📋 订单明细</div><div class="ant-table-wrap"><table class="ant-table"><thead><tr><th>订单编号</th><th>订单标题</th><th>类型</th><th>金额</th><th>状态</th><th>代练</th><th>创建时间</th></tr></thead><tbody>';
...
function rOrderMgmt(){
  ...
  var orders=[
    ...
    {id:'DD202605260011',title:'任务代打日常全清',status:'执行中',st:'ant-tag-primary',prog:'6/10',asset:'6M/10M',booster:'王代练',settle:'¥105',amt:'¥150',dep:'¥30',cs:'小李',src:'内部录入',studio:'-',contact:'137****6666',type:'代练',game:'三角洲',zone:'QQ',role:'玩家B',time:'05-26 13:20',guild:'三角洲机构A组'},
    ...
    {id:'DD202605250015',title:'排位黄金→铂金',status:'待结算',st:'ant-tag-danger',prog:'100%',asset:'5M',booster:'陈代练',settle:'¥245',amt:'¥350',dep:'¥70',cs:'小李',src:'内部录入',studio:'-',contact:'134****0003',type:'代练',game:'三角洲',zone:'QQ',role:'玩家E',time:'05-25 16:30',guild:'三角洲机构A组'},
    ...
    {id:'DD202605240020',title:'排位钻石→黑鹰',status:'执行中',st:'ant-tag-primary',prog:'30%',asset:'2M/12M',booster:'赵代练',settle:'¥560',amt:'¥800',dep:'¥160',cs:'小周',src:'上家抢单',studio:'上家XX',contact:'131****0005',type:'代练',game:'三角洲',zone:'QQ',role:'玩家G',time:'05-24 09:00',guild:'三角洲机构B组'},
    ...
    {id:'DD202605220005',title:'账号练级10→50',status:'已完成',st:'ant-tag-success',prog:'100%',asset:'50M',booster:'孙代练',settle:'¥686',amt:'¥980',dep:'¥196',cs:'小李',src:'上家抢单',studio:'上家YY',contact:'136****0008',type:'代练',game:'三角洲',zone:'QQ',role:'玩家J',time:'05-22 08:30',guild:'三角洲机构B组'},
...
function doGrab(rid,rtitle,ramt){
  ...
  if(isBooster){
    ...
    confirmBody+='<div style="margin-top:12px;"><div class="ant-form-label"><span class="req">*</span>分配代练</div><select class="ant-input" id="grab-assign-booster"><option>请选择代练</option><option>王代练（独立·好评98.5%）</option><option>陈代练（独立·好评99.1%）</option><option>赵代练（独立·好评97.3%）</option><option>孙代练（独立·好评95.8%）</option></select></div>';
...
function rGuildSettlement(){
  var rows=[
    {guild:'三角洲机构A组',admin:'张会长',tel:'138****1111',amt:'¥8,750.00',period:'2026.05.01 - 2026.05.28',time:'2026-05-28 10:30',account:'中国工商银行 6212****1234 张会长',status:'待打款',st:'ant-tag-warning'},
    {guild:'三角洲机构B组',admin:'李会长',tel:'139****2222',amt:'¥4,760.00',period:'2026.05.01 - 2026.05.28',time:'2026-05-28 14:15',account:'中国建设银行 6227****5678 李会长',status:'待打款',st:'ant-tag-warning'},
    {guild:'三角洲机构C组',admin:'王会长',tel:'137****3333',amt:'¥2,730.00',period:'2026.04.25 - 2026.05.25',time:'2026-05-25 09:00',account:'招商银行 6214****9012 王会长',status:'已打款',st:'ant-tag-success'},
    {guild:'三角洲机构A组',admin:'张会长',tel:'138****1111',amt:'¥12,500.00',period:'2026.04.01 - 2026.04.28',time:'2026-04-28 16:20',account:'中国工商银行 6212****1234 张会长',status:'已打款',st:'ant-tag-success'},
    {guild:'三角洲机构B组',admin:'李会长',tel:'139****2222',amt:'¥8,200.00',period:'2026.04.01 - 2026.04.28',time:'2026-04-28 11:00',account:'中国建设银行 6227****5678 李会长',status:'已打款',st:'ant-tag-success'}
  ];
  var h='<div class="filter-bar"><select class="ant-input" style="width:120px;"><option>结算状态</option><option>待打款</option><option>已打款</option></select><input class="ant-input" style="width:160px;" placeholder="机构名称"><button class="ant-btn ant-btn-primary ant-btn-sm">查询</button><button class="ant-btn ant-btn-sm">重置</button><button class="ant-btn ant-btn-sm" style="margin-left:8px;" onclick="exportGuildSettle()">导出 CSV</button></div>';
  h+='<div class="ant-card"><div class="ant-card-body np"><div class="ant-table-wrap"><table class="ant-table"><thead><tr><th style="width:120px;">操作</th><th>机构名称</th><th>管理员信息</th><th>结算金额</th><th>结算周期</th><th>发起时间</th><th>收款账户</th><th>结算状态</th></tr></thead><tbody>';
  rows.forEach(function(r){
  ...
  h+='</tbody></table></div></div></div>';
  h+='<div class="ant-alert">💡 机构代练结算规则：平台 → 机构统一打款 → 机构线下分发给所属代练员。独立代练由平台直接结算。</div>';
  return h;
}
...
function exportGuildSettle(){
  ...
  if(!table)return;
  table.querySelectorAll('tr').forEach(function(tr){var cells=[];tr.querySelectorAll('td').forEach(function(td){cells.push(td.textContent||'');});rows.push(cells);});
  var headers=['操作','机构名称','管理员信息','结算金额','结算周期','发起时间','收款账户','结算状态'];
...
// ── 用户管理 ──
var userList=[{uid:'U20260527001',name:'王代练',tel:'138xxxx1234',guild:'三角洲机构A组',pos:'代练',acct:'wang_dl'},{uid:'U20260527002',name:'刘代练',tel:'139xxxx5678',guild:'三角洲机构A组',pos:'代练',acct:'liu_dl'},{uid:'U20260527003',name:'陈代练',tel:'137xxxx9012',guild:'-',pos:'代练',acct:'chen_dl'},{uid:'U20260527004',name:'赵代练',tel:'136xxxx3456',guild:'三角洲机构A组',pos:'代练',acct:'zhao_dl'},{uid:'U20260527005',name:'孙代练',tel:'135xxxx7890',guild:'三角洲机构B组',pos:'代练',acct:'sun_dl'},{uid:'U20260527006',name:'张会长',tel:'138xxxx1111',guild:'三角洲机构A组',pos:'会长',acct:'zhang_hz'},{uid:'U20260527007',name:'小李',tel:'139xxxx0001',guild:'-',pos:'客服',acct:'li_xiaomei'},{uid:'U20260527008',name:'小周',tel:'139xxxx0002',guild:'-',pos:'运营',acct:'zhou_xiao'}];
var userNextId=9;
function rUserMgmt(){
